...
import pankaj.CandleStickStatus.db.DAO;
import pankaj.CandleStickStatus.db.DbModel;
import pankaj.CandleStickStatus.helpers.StringUtils;
...
public abstract class BaseDAO<T extends DbModel> implements DAO<T> {
	...
	try {
	    c = db.query(
				getTableName(), null, selection, selectionArgs,
	...
	public Cursor cursorRawQuery(String query) {
		Cursor c = null;
		c = db.rawQuery(query,null);
		return c;
	}
